...
public class EncounterParticipantService
{
    ...
    public EncounterParticipantService(IEncounterParticipantRepository participantRepository, IEncounterRepository encounterRepository, IEntityRepository entityRepository)
    {
    ...
    public Result<int> AddParticipantsToEncounter(string encounterName, IEnumerable<string> npc)
    {
        ...
        try
        {
            ...
            foreach (var spec in npc)
            {
                ...
                int existingCount = _participantRepository.GetParticipantCount(existingEncounter.Id, domainEntity.Id);
    ...
    private EncounterRecord? EnsureEncounterExists(string encounterName) =>  _encounterRepository.GetEncounter(encounterName);

    private EntityRecord? EnsureEntityExists(string entityName) =>
        _entityRepository.GetEntityByName(entityName, EntityType.npc);
    ...
    private EncounterParticipant BuildParticipant(int encounterId, Entity entity, int index)
    {
        ...
        return new EncounterParticipant
        {
            ...
            CurrentHp = int.Parse(entity.MaxHp!),
        };
    }
}