...
namespace gm_codex.Application.ConsoleUI.Party;
...
public class ReadPartyUi
{
    public static void ViewAllParties(Result<List<PartyRecord>> result)
    {
        ...
        foreach (var party in parties)
        {
            ...
            AnsiConsole.Write(
                new Panel(table).Header($"{party.Name}").Border(BoxBorder.None).Expand()
            );
    ...
    public static void ViewSingleParty(Result<Domain.Models.Party> result)
    {
        ...
        foreach (var member in result.Value!.Members)
        {
            memberTable.AddRow(
                member.Id.ToString(), 
                member.Name,
                member.Race.ToString(),
                member.SubRace ?? "-",
                member.EntityClass.ToString(),
                member.SubClass ?? "-"
                );
        }

        AnsiConsole.Write(
            new Panel(partyTable).Header($"{result.Value.Name}").Border(BoxBorder.None).Expand()
        );

        AnsiConsole.Write(new Panel(memberTable).Header("Members").Border(BoxBorder.None).Expand());
    }
}
